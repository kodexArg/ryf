	...
	<div class="flex h-11 space-x-3 w-fit">
		{#if $page.url.pathname !== '/'}
		  <button on:click={goSomeWhereBack} transition:fade={transitionConfig}>
			<Icon svg="back-arrow" class="w-11 h-11 p-1 text-primary-500 hover:text-primary-400 hover:scale-95" />
		  </button>
		{/if}
		<button on:click={togglePopup} on:mouseover={() => showPopup = true} class:scale-95={showPopup} class="w-11 h-11">
